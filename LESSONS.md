...
## 2026-07-30 — v0.19.6 清理全局技能硬编码 CLI 命令
...
### 本次改动统计
修改文件: 1 个 | 编译: BUILD SUCCESSFUL | 版本: 0.19.5 -> 0.19.6

---
## 2026-07-30 — v0.19.5 全局工具动态化 + 标签命名修复

### 101. 全局工具列表必须动态生成，不能硬编码
- 场景：框架设置的"全局工具"列表是硬编码的CLI命令列表，安装新插件后新命令不会出现。
- 修复：保留内置命令的curated描述，末尾动态追加已激活插件的命令（fs.cat/net.curl等）。
- 教训：**任何展示"全部XXX"的列表，数据源要么是文件系统扫描，要么是运行时注册表。硬编码列表一定会和实际情况脱节。**

### 102. 中英文标签要逐行核对，尤其是同名变量
- 场景：frameworkGlobalSkills 的中文标签被写成 "全局工具(Skills)"，和 frameworkGlobalTools 的 "全局工具(Tools)" 撞名。
- 后果：用户看到两个"全局工具"，不知道哪个是Tools哪个是Skills。
- 教训：**中英文 strings 配对时逐行检查。同名枚举值的中文标签不应该出现重复命名。**

### 本次改动统计
修改文件: 3 个 | 编译: BUILD SUCCESSFUL | 版本: 0.19.4 -> 0.19.5
