...
- 场景：plugin-self 的 4 条命令与内核 `self` 命名空间同名，`register()` 执行 `commands[fullName] = executor` 无冲突保护，后注册者覆盖先注册者——Agent 实际调用的可能是插件版而非内核版，职责归属不明。
- 处置：退役插件，`self` 完全归属内核；排查插件/内核命令冲突时从构建层移除，不依赖注册顺序。
- 教训：**新增插件命名空间前与内核命名空间做冲突清单核对；同名归属由构建决定，不靠注册时序**。

### 114. 按 agent 过滤的存储必须先持久化 agentName 字段
- 场景：EvolutionStore 初版 `EvolutionFailure` 未含 agentName，`recentFailures(agentName)` 过滤对持久化数据无效——JSONL 重读后无法分辨记录归属。
- 修复：数据类加入 `agentName` 字段，落盘即携带。
- 教训：**多租户存储设计先想"这条数据从文件读回来还能不能回答过滤条件"**。

### 115. 单元测试构造场景要与实现的分级语义一致
- 场景："轻引导应简短"测试调 `buildFragment("evo-test-6", "fs.cat", "boom")`，但 store 最新记录是另一 agent 的 deep 失败——实现按"最新失败记录"分级，测试没构造出"当前失败=轻失败"的状态，断言失败。
- 修复：换独立 agent 构造一次失败（轻）；重复同 key 构造复现（深）。
- 教训：**先读实现的分级依据（什么数据决定什么行为），再构造数据**。
...
- 场景：v0.19.5 声称"移除硬编码"，实际 skillMgmt 那 10 条 CLI 命令并没删掉，release 是个空壳。
- 根因：改完代码后没确认改动生效就打了 tag 发了 release。
- 教训：**commit → grep 确认改动 → build → install 验证 → tag → release。跳过确认步骤的 release 等于没发。**

### 本次改动统计
修改文件: 1 个 | 编译: BUILD SUCCESSFUL | 版本: 0.19.5 -> 0.19.5
...
- 场景：双层 Skills 池上线后，Agent 不知道"优先查本地，找不到再查全局"这个行为规则。
- 修复：PromptEngine 中新增 Skills 双层池引导段。
- 教训：**每次增加 CLI 命令或改变数据流时，必须同步检查系统提示词是否需要更新。Agent 认知层和代码层是两套独立的"真理"。**

### 92. 不再替用户选择——UI 只做输入容器
- 场景：CRON/SCHEDULE 对话框有预设选项，用户说"时间参数让 Agent 算"。
- 修复：去掉所有预设 RadioButton，改为纯输入框 + 提示引导找 Agent。
- 教训：**对话框里的预设会误导用户绕过 Agent 直接操作底层参数。UI 应该引导用户用自然语言和 Agent 对话，而不是自己做配置。**

### 93. 插件 UI 按钮暴露给用户前先问"这有意义吗"
- 场景：clipboard 的"粘贴"按钮在底部弹窗出现，但它是 Agent 内部命令。
- 修复：ClipboardPlugin.uiButtons = emptyList()。
- 教训：**PluginUiButton 注册前问：这个人点下去有用吗？只对 Agent 有用的东西不要注册 UI 按钮。**

### 94. Mission+->FLEET 重命名：名字要传达差异
- 场景：Mission+ 和 Mission 走完全相同的代码路径，名字里的"+"没有意义。
- 修复：改为 FLEET（舰队），描述改为"多 Agent 编队协同"，预设未来跨设备分发能力。
- 教训：**模式的名字必须让用户（和 Agent）一看到就知道它和别的模式有什么不同。**

### 95. 原创标识应该按功能标记，不按类别批量刷
- 场景：最初对所有 ItemCategory.BUILTIN 加 WowBlue 徽标，但 self.status 这类通用命令不该标。
- 修复：逐个标记 sys.*、agent.memory、agent.dream、self.trigger 等确实为 WowBlue 原创的功能。
- 教训：**标识的关键是准确，不是覆盖率。标错了比不标更让人困惑。**

### 本次改动统计
修改文件: 16+ 个 | 编译: BUILD SUCCESSFUL | 版本: 0.19.1 -> 0.19.2
