...
object AcpHolder {
    val server = com.mengpaw.core.acp.AcpServer(com.mengpaw.core.agent.AgentProfile())
    var transport: com.mengpaw.core.acp.AcpHttpTransport? = null
}
...
object SelfExecutor {
    ...
    private val ACP_SUBCOMMANDS: Map<String, suspend (List<String>, ExecutionContext) -> ExecutionResult> = mapOf(
        "start" to { _, _ ->
            val transport = com.mengpaw.core.acp.AcpHttpTransport(AcpHolder.server)
        ...
        "pair" to { a, _ ->
            if (a.size < 3) ExecutionResult.fail("Usage: self.acp pair <device-id> <peer-fingerprint>\n获取对方指纹: 让对方执行 self.acp fingerprint", errorCode = ErrorCodes.ERR_INVALID_INPUT)
                ...
                val myFp = com.mengpaw.core.acp.AcpCrypto.myFingerprint()
                val peerFp = a[2]
                com.mengpaw.core.acp.AcpCrypto.deriveKey(myFp, peerFp, a[1])
                com.mengpaw.core.security.PromptFirewall.trust(a[1], peerFp)
                ExecutionResult.ok("已配对设备: ${a[1]}\n加密: AES-256-CBC (密钥已派生)\n该设备现在拥有完整访问权限。")
            }
        },
        "fingerprint" to { _, _ ->
            ExecutionResult.ok("本设备指纹: ${com.mengpaw.core.acp.AcpCrypto.myFingerprint()}\n将此指纹提供给配对方。")
        },
        "untrust" to { a, _ ->
            if (a.size < 2) ExecutionResult.fail("Usage: self.acp untrust <device-id>", errorCode = ErrorCodes.ERR_INVALID_INPUT)
            else { com.mengpaw.core.security.PromptFirewall.untrust(a[1]); ExecutionResult.ok("已解除配对: ${a[1]}") }
        },
        "trusted" to { _, _ ->
            val list = com.mengpaw.core.security.PromptFirewall.listTrusted()
            if (list.isEmpty()) ExecutionResult.ok("(无已配对设备)\n\n配对: self.acp pair <device-id>")
            else ExecutionResult.ok("已配对设备:\n" + list.joinToString("\n") { "• $it" })
        },
        "firewall" to { _, _ ->
            ExecutionResult.ok(com.mengpaw.core.security.PromptFirewall.guestPolicySummary())
        }
    )
    ...
    private val TRIGGER_SUBCOMMANDS: Map<String, suspend (List<String>, ExecutionContext) -> ExecutionResult> = mapOf(
        "add" to { a, _ ->
            if (a.size < 5) ExecutionResult.fail("Usage: self.trigger add <cron|lifetime> <id> <expr> <action>", errorCode = ErrorCodes.ERR_INVALID_INPUT)
                ...
                val engine = com.mengpaw.core.trigger.TriggerEngine
        ...
        "list" to { _, _ ->
            val triggers = com.mengpaw.core.trigger.TriggerEngine.list()
        ...
        "remove" to { a, _ ->
            com.mengpaw.core.trigger.TriggerEngine.remove(a.getOrElse(1) { "" })
            ExecutionResult.ok("Removed.")
        },
        "topics" to { _, _ ->
            ExecutionResult.ok("## 真人感话题\n\n${com.mengpaw.core.trigger.TriggerEngine.LIFETIME_TOPICS.joinToString("\n") { "- $it" }}")
        },
        "cron-wake" to { _, _ ->
            com.mengpaw.core.trigger.TriggerEngine.registerCronAlarm(null)
    ...
    private suspend fun mcp(args: List<String>, ctx: ExecutionContext): ExecutionResult {
        if (args.isEmpty()) return ExecutionResult.ok(com.mengpaw.core.mcp.McpClient.statusReport())
        val sub = args[0]
    ...
    private val MCP_SUBCOMMANDS: Map<String, suspend (List<String>, ExecutionContext) -> ExecutionResult> = mapOf(
        "connect" to { a, _ ->
            if (a.size < 2) ExecutionResult.fail("Usage: self.mcp connect <id>", errorCode = ErrorCodes.ERR_INVALID_INPUT)
                ...
                val cfg = com.mengpaw.core.mcp.McpClient.PRESETS[a[1]]
                if (cfg == null) ExecutionResult.fail("Unknown preset: ${a[1]}. Available: ${com.mengpaw.core.mcp.McpClient.PRESETS.keys}", errorCode = ErrorCodes.ERR_NOT_FOUND)
                else { com.mengpaw.core.mcp.McpClient.connect(a[1], cfg); ExecutionResult.ok("Connected to ${cfg.name}.") }
            }
        },
        "disconnect" to { a, _ ->
            if (a.size < 2) ExecutionResult.fail("Usage: self.mcp disconnect <id>", errorCode = ErrorCodes.ERR_INVALID_INPUT)
            else { com.mengpaw.core.mcp.McpClient.disconnect(a[1]); ExecutionResult.ok("Disconnected: ${a[1]}") }
        },
        "status" to { _, _ -> ExecutionResult.ok(com.mengpaw.core.mcp.McpClient.statusReport()) },
        "call" to { a, _ ->
            if (a.size < 4) ExecutionResult.fail("Usage: self.mcp call <connection-id> <tool-name> <args-json>", errorCode = ErrorCodes.ERR_INVALID_INPUT)
                ...
                val conn = com.mengpaw.core.mcp.McpClient.listConnections().find { it.id == a[1] }
    ...
    private suspend fun theme(args: List<String>, ctx: ExecutionContext): ExecutionResult {
        val themeFile = java.io.File(com.mengpaw.core.DataPaths.AGENTS, "theme.md")
