...
class AgentDocManager(
    ...
    private val baseDir: String = com.mengpaw.core.DataPaths.AGENTS
) {
...
""".trimIndent()
    ...
    fun regenerateCliDoc(pluginManager: PluginManager) {
        file(AgentDocType.CLI).writeText(buildString {
            ...
            appendLine("| self.theme | self.theme primary=#165DFF surface=#FFFFFF | 修改主题色 | 无 |")
            appendLine()
            ...
            appendLine("- `plugin.auto sleep-idle` — 休眠所有非核心插件（self/plugin/agent/pad 除外）")
            ...
            appendLine("| pad-plugin | PAD悬浮窗 | pad.* (2命令) | 悬浮窗权限 |")
