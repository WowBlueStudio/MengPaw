    ...
    private fun handleBrowserExtract(url: String, title: String) {
        ...
        try {
            ...
            taskFile.writeText(buildString {
                ...
                appendLine()
                appendLine("## 执行步骤")
                appendLine("1. 抓取转档: search.md $url --name article_$taskId (search 命令不可用时先 plugin.install browser-search-plugin; 仍不可用则用 net.curl $url 抓取正文)")
                appendLine("2. cat <第1步返回的md路径>             — 阅读全文 (长文用 head/tail/sed -n 取片段)")
                appendLine("3. 提炼要点: 一句话总结 + 3~6 条核心要点 + 1 条关键数据或引用")
                appendLine("4. 写回传文件 (多行内容用 printf 逐个换行写入, 或先写到临时文件再 cat 合并):")
                appendLine("   printf '# 标题\\n\\n[原文链接]($url)\\n\\n## 核心要点\\n- 要点1\\n- 要点2\\n\\n## 总结\\n一段话\\n' > ${inbox.absolutePath}/browser_return_$taskId.md")
                appendLine("   （格式要求: # 标题 / [原文链接](URL) / ## 核心要点 (列表) / ## 总结 (一段话)）")
                appendLine("5. cat 回读浏览器回传文件确认落盘无误")
                appendLine("6. rm ${taskFile.absolutePath}   — 删除本任务文件, 防止重复执行")
                appendLine("7. Final Answer 告知用户: 提炼完成, 浏览器将自动弹出预览")
            })
            // 直接触发; 任务文件保留到 Agent 执行完成 (脚本步骤 6 由 rm 删除, 防重复执行)
