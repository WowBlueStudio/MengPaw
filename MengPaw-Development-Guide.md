...
**气泡 UI 独立嵌套 (v0.42.2, 用户需求)**: 思考过程容器不再整体折叠 — 每轮 `ProcessStep` 渲染为独立嵌套气泡 (`ThinkingStepBubble`: "第 N 轮思考" 折叠头 + 思维链全文 + 该轮工具块), 工具块 (`ProcessToolRow`) 各自独立折叠 — **折叠工具调用不再连带折叠思维链**; 运行中强制展开全部思考气泡, 最终答案开始后逐轮默认收起 (用户手动状态跨重组保留); 容器顶部加 Agent 头 (与最终答案气泡一致)。滚动行为沿用 `MainScreenScrollBehavior`: 生成期间每 100ms 贴底跟随, 用户上滑立即冻结, 回到底部自动恢复, 程序滚动被手势打断有兜底冻结。
