...
- **索取**: `skill.ls --agent <Agent名>` 浏览他人本地技能; `skill.request <技能名> <来源Agent>` 复制到本地并补 `## 来源` 标记。冲突以 **description 标准化相同**为准 (同名文件或简介相同均中止, 不覆盖)。先只做同设备, 跨设备复用 fleet/ACP 通道挂后续; 全局分享仍走显式 `skill.push`。`skill.enable/disable` 按 `skill.run` 查找顺序**先本地后全局** (本地索取产物可直接启停), 未找到技能返回错误。
