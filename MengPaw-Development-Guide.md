...
### 3.7 测试 (16 模块 784 测试，v0.34.1+Codex 维护实测快照：kernel 428 + core 45 + shell 63 + browser 17 + 插件 231，0 failures)

| 模块 | 测试数 | 覆盖 |
|------|-------|------|
| mengpaw-kernel | 428 | ACP 信任/防火墙、PromptEngine 解析/循环检测、附件二进制挂载/指纹缓存 (多模态重发成本)、会话压缩/恢复 (SessionManager 30 用例拆 6 文件)、命令注册、swarm/mission、PinnedSkills 清单 (原子写/损坏降级/路径消毒)、pinned 指针注入 (尾插/指纹失效/缺失降级)、**高危门禁多行引号保护 + 进化闭环/幻觉门禁(静默·无上限拒绝)/回合内重试循环停指令/读回验证 (P0/P1/P4, 2026-08-08)** |
