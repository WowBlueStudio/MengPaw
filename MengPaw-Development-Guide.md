...
> v0.40.4 增量（实测）：kernel 577 → 591（SseStreamParserTest 9 用例首次直测 consumeSseStream + LlmPayloadTest 5 用例：非流式 reasoning 分离 / 请求体不回显思维链），shell 双套 208 → 210（BubbleStreamCoordinatorTest 11 → 12：思维链与正文交错到达同轮完整显示），总数 1425 → 1441，0 failures。

| 模块 | 测试数 | 覆盖 |
|------|-------|------|
| mengpaw-kernel | 591 | ACP 信任/防火墙、PromptEngine 解析/循环检测、附件二进制挂载/指纹缓存 (多模态重发成本)、会话压缩/恢复、命令注册、swarm、PinnedSkills 清单、pinned 指针注入、高危门禁/进化闭环/幻觉门禁/Fleet 委派/能力收集 (v0.35.5) + **PluginRuntimeLoader dex 容器检查/plugin-class 清单 (v0.35.6 新增 4 用例)** + CommandMonitor/Linux 通道 (v0.36) + evaluateRulesOnly 规则审查 (v0.36.3 新增 4) + **SseStreamParserTest 9 + LlmPayloadTest 5（v0.40.4 全厂商思维链解析直测）** |
