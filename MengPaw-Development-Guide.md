...
> v0.46.1 发布实测（2026-09-01，token 用量记录重构 + 并发安全 + 更新源修复）：kernel 643 + core 116 + shell 236 + 插件 546 = **1541 用例**，0 failures。增量：TokenUsageRegistryTest +2（onUsage 统一记录回调注入/未注入 × 双套；口径: kernel 641 → 643）。用量记录下沉内核 `TokenUsageRegistry` 根治多模式统计缺失；`TokenStatsCollector` 加 `@Synchronized` 并发安全；UpdatePlugin browser 源取最高版本。
> v0.46.2 发布实测（2026-09-10，DeepSeek 思考强度四档 + 空响应根因修复 + 模型选择链路修复）：kernel 643 → 650（+7）+ core 116 + shell 236 → 248（+12 双套）+ 插件 546 = **1560 用例**，0 failures。增量：LlmPayloadTest +6（四档注入 Max/High/Low 带 `thinking.type=enabled` + `reasoning_effort` / Off 只发 disabled / 不传档位不带 thinking 字段 / 仅 DeepSeek 端点注入 / 默认 High 与未知持久化值回退）+ AdaptiveLlmProviderTest +2（`AdaptiveConfig.forEndpoint` 思考型端点 16K 与其余 4096 / 构造默认随端点自适应且显式 config 优先）；SettingsRemoteTest 4（模型列表探测 URL 派生：DeepSeek 官方 `/models` 先于 `/v1`、已带 `/v1` 不产生 `/v1/v1`、DashScope/GLM/火山保留版本段、空端点与尾斜杠容错）+ SettingsModelsPresetTest +2（DeepSeek 预置端点含视觉型号 / 思考档位默认 High 且仅 DeepSeek 可切换）。

| 模块 | 测试数 | 覆盖 |
|------|-------|------|
| mengpaw-kernel | 650 | ACP 信任/防火墙、PromptEngine 解析/循环检测、附件二进制挂载/指纹缓存 (多模态重发成本)、会话压缩/恢复、命令注册、swarm、PinnedSkills 清单、pinned 指针注入、高危门禁/进化闭环/幻觉门禁/Fleet 委派/能力收集 (v0.35.5) + **PluginRuntimeLoader dex 容器检查/plugin-class 清单 (v0.35.6 新增 4 用例)** + CommandMonitor/Linux 通道 (v0.36) + evaluateRulesOnly 规则审查 (v0.36.3 新增 4) + **SseStreamParserTest 17 + LlmPayloadTest 11 + RemoteApiTest 5（v0.40.4 全厂商思维链解析直测 + 2026-08-17 五家官方流式夹具）** + PromptEngineTest sys 权限前置 (v0.42.3, +1) + DeepSeek 思考模式回传 (v0.42.1, +5) + 对话需求跟踪 (v0.42.1, +6) + RiskGateTest 无障碍命令分级 (v0.42.2, +1) + **LoopDetector 三通道/AgentErrors/JSON 数组/GoalSessionStore/RalphRunner (v0.43.0, +17)** + **EvolutionQueue/G2 复现计数跨重启 (v0.44.0, +5)** + **思考强度四档注入/端点过滤/默认回退 (v0.46.2, +7)** |
...
**气泡 UI 重构 (v0.34.3)**: 时间轴主导 — 一次任务 = **思考过程容器** (`ChatMessageUi.ThinkingProcess`, 单一可折叠, 跨所有轮次) + **最终答案气泡** (`ChatMessageUi.FinalAnswer`, 独立)。执行中: 思考写入容器 → 完整 `Action:` 行出现 → 工具完成挂工具行 (只显示命令名, 失败红字, 点击展开参数+观察全文) → 检测到 `Final Answer:` (onDelta 原始增量累积检测) 时**过程容器自动折叠** + 答案气泡流式。写入器 `ThinkingProcessWriter` 替代旧 `StepBubbleWriter` (每轮一卡的模型废除); 历史会话经 `reflowLegacyMessages` 渲染层统一重排 (旧 agent_step/agent_trace 序列合并为过程容器+答案, 持久化格式兼容, 新增 thinking_process/final_answer 类型)。**闭环兜底 (v0.36.2)**: `PromptEngine.parse` 规则 3/4 对无 `Final Answer:` 标记的纯文本自然回答/Thought-only 也判为最终答案 — 此类输出流式检测永不命中, 引擎 `run()` 返回后 Shell 兜底 `beginFinalAnswer()` 强制闭环 (折叠容器 + 创建答案气泡), 防止思考容器 `isRunning` 永 true (自动折叠失效/滚动回收后手动折叠被覆盖)。

**思考气泡层级定案 (2026-08-18 用户需求, 推翻 v0.42.2 逐轮折叠, 未发布)**: 思考过程容器恢复**整体一级折叠** — 折叠头「思考过程」收起答案前的全部过程 (跨所有轮次, 不显示轮次/调用计数), 运行中强制展开, 最终答案开始后默认收起 (手动状态跨重组保留); 每轮 `ProcessStep` 渲染为独立气泡 (`ThinkingStepBubble`), **无折叠头** (不做二级折叠), 思维链全文直展; 工具调用+调用结果每组二级折叠 (`ProcessToolRow`, 默认收起, 无气泡底色); 思考区 (Agent 头/折叠头/每轮气泡) 相比回答气泡左右各缩进 8dp。**滚动修复 (v0.42.2, 用户反馈"生成时不动 + 结束后跳回上一条开头")**: `MainScreenScrollBehavior` 贴底改为 `scrollToBottom` — `scrollToItem` 定位末条 + `scroll` 补滚到真正末端 (原 `animateScrollToItem(total-1)` 对末条高于视口只对齐 item 顶部, 流式长答案时视口钉在开头 → "生成时不动"); 删除过时的"思考结束回顶 + 自动聚焦" — 原实现 `indexOfLast { Agent || AgentWithTrace }` 在主路径 `ThinkingProcess/FinalAnswer` 下命中历史旧 Agent 消息 → 结束跳回上一条开头。生成结束保持贴底; 用户上滑冻结跟随, 回到底部自动恢复。

**简化显示流程 (v0.40.1 定案 → v0.40.2 重构)**: 取消思考阶段的逐字流式打字机 — 工具轮在流式检测到完整 `Action:` 行时**一次性**显示整轮思考 (思考先出现, 不逐字); 工具行在 `onStep` (工具完成) 时按顺序挂入并带观察; 纯思考轮 (无 Action) 在 `onStep` 时一次性显示; 截断路径 (引擎异常未走 onStep) 由 `finish()` 兜底一次性显示思考 + 无观察工具行。最终答案轮保留流式输出 (打字机), 收到 `Final Answer:` / 引擎返回兜底时折叠思考容器 + 创建答案气泡。**v0.40.2 重构** (v0.40.1 实现失败, 用户复现三症状: "思考中… xxs 单独气泡计时不停 / 一轮思考后停止无最终答案 / 思考过程不展开"): 删除轮次队列 / `skipRound` / `snapshotRounds` / `flushText` / `thoughtShown` / `stepClosed` / `finalAnswerRoundId` 六套状态 — `StreamPlaybackBuffer` 只保留当前轮累积 + 完整 Action 行扫描 + 最终答案流式文本, 思考轮不再经过播放协程; `BubbleStreamCoordinator` 收敛为三件事 (思考一次性显示 / 工具行 onStep 挂观察 / 最终答案**无条件**流式) — 播放协程不再要求 raw 含 "Final Answer:" 字样, 纯文本最终轮 (parse Rule 3/4) 经引擎返回兜底后由播放器把整轮文本流式送进答案气泡, 不再误写进思考容器; `ThinkingProcessWriter` 删除 `RunningStepTracker` (index/ref 身份追踪 — 列表重建/恢复后身份失效, 是运行态残留的结构性诱因), 改按类型定位最后一条 (任务串行恒为本任务), `beginFinalAnswer`/`finalize` 幂等 — 任何路径 (含异常/截断/进程死亡恢复) 都终止运行态, 不残留 "思考中… Ns" 计时气泡; UI 运行中强制展开思考容器 (isRunning 优先)。`computeStreamDisplayText` 的标记判定全部**行首锚定** (`(?m)^\s*Final Answer[:：]` 等): 思考/正文里出现 "Final Answer:" / "Action:" / "Thought:" 字样 (复述用户原话) 不再误截断, 与协调器 Final Answer 检测同一口径。

**DeepSeek thinking mode 思维链分流 (v0.40.3, 用户定位根因)**: v0.40.1/0.40.2 的三症状真正根因是 **`reasoning_content` (思维链) 被 SSE 解析层当普通 content 增量透传** — DeepSeek 思考模式下流式响应先发思维链 (纯文本, 无 ReAct 标记), 后发 `content`。思维链进入 UI 流式缓冲后: ① 思维链里出现行首 "Final Answer:" (自拟答案草稿) → 误判为最终答案轮 → 后续工具轮被吞, "一轮思考后停止无最终答案"; ② 思维链里的 "Action:" 行 → 误宣布工具/显示错乱; ③ 真正的思维链从未被当作"思考"展示 → "思考过程不展开"; ④ 纯文本最终轮 (无标记) 时整轮 raw (思维链+答案) 被送进答案气泡。修复 (对齐 [DeepSeek thinking mode 文档](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode)): `SseStreamParser.consumeSseStream` / `RemoteApi` 把 `reasoning_content` 增量经**独立 `onReasoning` 回调**分流 (绝不混入 `onToken`/fullContent); 签名链 `LlmProvider.completeStreamingWithMessages` → `AgentEngine.run*/runReActLoop` → `AgentReActLoop` → shell `BubbleStreamCoordinator.onReasoning` 全线透传; 协调器把思维链累积为当前轮思考一次性显示 (思维链比 content 的 "Thought:" 更完整, 后者不再覆盖), 思维链**不参与** Action 扫描 / Final Answer 检测 / 最终答案流式 — 三症状全部消除。UI 等待态指示器同时改为纯 spinner (删除 "思考中… Ns" 计时文本, 用户定案)。

**全厂商思维链分流 (v0.40.4, 字段以各厂商官方 API 文档为唯一准则)**: v0.40.3 仅覆盖 DeepSeek 的 `reasoning_content`; 本项目支持的全部厂商思维链格式各不相同 — 新增 `ReasoningExtractor` 统一提取: OpenAI 兼容 delta/message 键序 `reasoning_content` (官方口径: DeepSeek [思考模式文档](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode) reasoning_content 与 content 同级 / Kimi platform.kimi.com delta.reasoning_content 与 message.reasoning_content / GLM-Z.AI docs.z.ai delta.reasoning_content / Qwen docs.qwencloud.com 两阶段流式 / 豆包 docs.volcengine.com reasoning_content / xAI docs.x.ai message.reasoning_content) → `reasoning` (Ollama 新版 /v1, 官方 /v1 文档未记载, 兼容兜底) → `thought`/`thinking` (自建兼容服务兜底), 只认字符串安全跳过; Anthropic 兼容 `content_block_delta` 的 `delta.type=="thinking_delta"` → `delta.thinking` (platform.claude.com 官方), `signature_delta` 忽略不回放。`RemoteApi` 删除重复 SSE 循环直接复用 `consumeSseStream` — 主链与 fallback 链同口径 (含首 token 前断流统一抛 LlmApiException); `parseBody` 改为返回 `ParsedLlmBody(content, reasoning, usage)`, 非流式 `message.reasoning_content` 独立提取到 provider 新增 `lastReasoning` (供观测), 思维链绝不拼进正文。协调器 `flushReasoning` 允许同轮重推覆盖 (交错到达不丢迟到思维链增量); 思维链**不参与** Action 扫描 / Final Answer 检测。**请求侧回传 (v0.41.1 未发布)**: DeepSeek 思考模式 OpenAI 兼容端点要求 assistant 的 `reasoning_content` 必须原样回传 — 官方 [Copilot CLI 集成页](https://api-docs.deepseek.com/zh-cn/quick_start/agent_integrations/copilot_cli/) 明示 openai 类型会触发 400 "The reasoning_content in the thinking mode must be passed back to the API"; 原定案"仅响应侧解析、不回传"导致多轮工具调用任务后段 400/行为漂移 (用户复现: 前几次单轮对话正常, 后几次多轮工具调用折叠/格式判断混乱)。修复: `Message.reasoning` 随 assistant 消息落历史 → `getStructuredHistory` 附 `reasoning_content` 键 → `buildRequestBody` **仅 deepseek 端点**透传 (OpenAI 等端点不接受该字段); 无工具调用轮次 DeepSeek 官方明确忽略该字段, 统一回传安全。Kimi 保留式思考回传仍属请求侧未实现范围。

**MiniMax 接入 (v0.41.0, 官方文档为唯一准则)**: 新增 MiniMax (稀宇科技) 预置 — 端点 `https://api.minimaxi.com/v1/chat/completions`, Bearer 认证, 模型 MiniMax-M3 (1M 上下文, 旗舰) / M2.7~M2 全系。思维链有两种官方形态: ① 默认 (不注入 `reasoning_split`) 时 thinking **内联在 `content` 的 `<think>...</think>` 标签内** — 流式经 `ThinkTagSplitter` 逐增量剥离 (含标签跨 chunk 拆分, 尾部保留标签前缀待补全), 非流式经 `ReasoningExtractor.stripThinkTags` 剥离, 剥离内容都进 `onReasoning`/`ParsedLlmBody.reasoning`, 标签丢弃; ② `reasoning_split=true` 时经 `reasoning_content` + `reasoning_details` 数组返回 (每项 `{type,id,format,index,text}`), **流式 delta 的 text 为当前块累计全文** (官方 OpenAI SDK 示例按 `len(reasoning_buffer)` 取新增) — 流式侧必须按全文 buffer 增量去重, 直接拼接会整段重复推送。按"仅响应侧解析"定案不注入 `reasoning_split`, 主路径是 `<think>` 剥离; `reasoning_details` 仅在自建网关/自定义端点触发时兜底。官方明确 M3 "当前不支持音频输入" (支持图片/视频), 故不列入 `VoiceCapability`。
