...
```
mengpaw/
├── mengpaw-core/              # 微内核 (Library AAR)
│   ├── cli/                   # CLI 引擎 (解析→安全策略→执行→审计)
│   ├── security/              # 安全层 (Vault/Sanitizer/Policy/IntegrityGuard/Firewall)
...
│   ├── acp/                   # Agent Communication Protocol (AES-256-CBC)
│   ├── trigger/               # Cron + 真人感触发器
│   ├── namespace/             # 内置命名空间 (self/sys)
│   └── DataPaths.kt           # 统一数据路径常量
│
...
```bash
./gradlew :mengpaw-core:testDebugUnitTest   # 86 单元测试
```
