...
3. **对齐可移植规范**（派生与手动创建同源约束）：技能是团队/其他 Agent 可复用的资产——
   - **完善**：步骤完整 + 验证规则，他人拿到可独立执行
   - **中性**：不掺个人偏好，不绑定特定设备/环境
   - **通用**：具体项目名/专名用 `{{占位符}}` 抽象，保留可复用骨架
   - **无敏感**：不含 API Key、令牌、真实路径、个人信息
4. 用自然语言向用户描述计划（含进化目标），等确认。**不要直接创建空白技能**
