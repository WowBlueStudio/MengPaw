...
source: core
---
# 表单自动化 (经 am 桥 page.* 命令面)

> 通道: am 桥单通道 `page.*` 命令 (推荐)。9880 桥 / MCP 工具已退役 (v0.9.0)。
> 主手册: `skill.run browser-control`。
...
```
page.eval "JSON.stringify({errs:document.querySelectorAll('.error').length,ok:!!document.querySelector('.success')})"
```
...
- 敏感操作前先 `page.content` 核对页面状态
- 提交后必须验证（跳转/title/DOM 状态），不要假定成功

## 进化目标

- 目标: 覆盖常见表单自动化全场景——登录/向导/选择/验证/错误恢复
- 稳定锚点: am 桥 page.* 调用格式（selector+text/script）与「提交后必须验证」原则
