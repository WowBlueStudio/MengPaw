...
| `external-plugins-status.md` | 23 KB | **外置插件现状与建议**（2026-09-10 快照）：12 个在售 + 2 个已退役插件逐个体检（命令/测试/缺陷）、宿主↔外置衔接机制、P0/P1/P2 横向问题清单、待决策项与建议行动顺序 | 管理外置插件（`mengpaw-connectors`）、做插件级发布、排查插件不可用/命令名对不上时 |
...
## 技能沉淀（`~/.agents/skills/`，权威；项目内 `.claude/skills/` 副本已于 2026-09-10 退役，git 历史可溯）
