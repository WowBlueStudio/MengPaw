...
- **Final Answer 门禁**：本轮有失败但最终回答未如实提及（`unmentionedFailures`：错误码 或 命令名+失败词 均未出现）→ 框架**拒绝接受** Final Answer，注入失败清单（命令 + Error [CODE]）强制重写；最多拒绝 2 次防 LLM 反复幻觉死循环（超限放行，失败清单已在历史可追溯）。位置 AgentReActLoop isFinal 分支，检测逻辑与幻觉率统计共用 `isFailureMentioned` 单点。
