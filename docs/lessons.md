...
- **Final Answer 门禁（静默）**：本轮有失败但最终回答未如实提及（`unmentionedFailures`：错误码 或 命令名+失败词 均未出现）→ 框架**拒绝接受** Final Answer，反馈**只注入下一轮 LLM 请求**（buildConversation 末尾追加 system，不写会话历史——UI/持久化/后续上下文零污染），引导 Agent 优先静默纠正（重试/换命令，成功则正常收尾）或自然语言如实说明，不再强制堆内部错误码；最多拒绝 2 次防死循环（超限放行）。**失败已弥补豁免**：同命令同参数重试成功 → 从待提及清单移除（换参数 = 不同操作不豁免），幻觉率统计同步只计最终仍失败的条目。位置 AgentReActLoop isFinal 分支，检测逻辑与幻觉率统计共用 `isFailureMentioned` 单点。
